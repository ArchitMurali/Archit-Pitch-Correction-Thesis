...
def detect_pitch(f,W,t,sample_rate,bounds):
    ACF_Vals = [ACF(f,W,t,i) for i in range(*bounds)] # List of ACF Computed at all lag values within bounds
    sample = np.argmax(ACF_Vals) + bounds[0] # Finding the index of the maximum values
    return sample_rate / sample # Returning the detected frequency
